...
impl Into<BoardState> for &Board{
    fn into(self) -> BoardState {
        BoardState(
            self.guesses.iter().map(|word| {
                word.chars().enumerate().map(|(index, character)| {
                    let word_chars = self.word.chars();
                    let state = if let Some((i, _)) = word_chars.enumerate().find(|x| &x.1 == &character){
                        if i == index{
...
impl Board{
    ...
    pub fn reset_with_length(&mut self, len : usize){
        let word = self.data.0.iter().filter(|(k, _)| k.len() == len).nth(rand::random_range(0..len)).unwrap().0.to_string();
        self.word = word;
        self.guesses.clear();
    }
